...
public class HerramientasActasServiceImpl implements HerramientasActasService {
	...
	public void delete(Herramientasacta entity) throws Exception {
		...
		if (herramientasActaRepository.existsById(entity.getIdherramienta()) == false) {
			throw new Exception(
					"La herramienta Acta con id " + entity.getIdherramienta() + " No existe (No se puede eliminar)");
		}
	...
	public Herramientasacta guardarHerramientasDelActa(HerramientasDelActaDTO herramientasDelActaDTO) throws Exception {
		...
		Integer idreuniones = entradActaRepository.buscarIdReunion(herramientasDelActaDTO.getIdproyecto());

		// Problema detectado, la acta corresponde a un unico valor
		// y ese unico valor va ligado a la primera reunion por ende, es necesario
		// ubicar ese id unico
	...
	public List<Herramientasacta> BuscarDatosDeHerramietasPrevias(HerramientasDelActaDTO herramientasDelActaDTO)
			throws Exception {
		// return
		// herramientasActaRepository.BuscarDatosDeHerramietasPrevias(herramientasDelActaDTO.getIdfase(),herramientasDelActaDTO.getIdproyecto());
	...
	@Override
	@Transactional(readOnly = false, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public Herramientasacta actualiazrHerramientasDelActa(HerramientasDelActaDTO herramientasDelActaDTO)
			throws Exception {
		// Integer idreuniones = entradActaRepository.buscarIdReunion(herramientasDelActaDTO.getIdfase(),herramientasDelActaDTO.getIdproyecto());
		Integer idreuniones = entradActaRepository.buscarIdReunion(herramientasDelActaDTO.getIdproyecto());
		// --------------------------------------------------
		...
		if(acta == null) {
			throw new Exception("Fallo controlado -> Error 401");
		}
		Herramientasacta entity = new Herramientasacta();
		entity.setActas(acta);
		entity.setEstado(true);
		entity.setHabilidades(herramientasDelActaDTO.getHabilidades());
		entity.setHerramientareuniones(herramientasDelActaDTO.getHerramientareuniones());
		// entity.setIdherramienta(herramientasDelActaDTO.getHerramientareuniones());
		entity.setJuicioexpertos(herramientasDelActaDTO.getJuicioexpertos());
		entity.setParticipa(herramientasDelActaDTO.getParticipa());
		entity.setRecopilaciondatos(herramientasDelActaDTO.getRecopilaciondatos());
